...
public class ViewFinderView extends View {
    ...
    public void drawViewFinderMask(Canvas canvas) {
        ...
        canvas.drawRect(0, 0, width, mFramingRect.top, paint); // Top frame
        canvas.drawRect(0, mFramingRect.top, mFramingRect.left, mFramingRect.bottom + 1, paint); // Left frame
        canvas.drawRect(mFramingRect.right + 1, mFramingRect.top, width, mFramingRect.bottom + 1, paint); // Right frame
    ...
    public synchronized void initFramingRect() {
        ...
        if (screenResolution == null || mFramingRect != null) {
            return;
        }
        
        initFramingRect(screenResolution.x, screenResolution.y);
    }

    public synchronized void initFramingRect(int canvasWidth, int canvasHeight) {
        int width;
        int height;
        int orientation = DisplayUtils.getScreenOrientation(getContext());

        if(orientation != Configuration.ORIENTATION_PORTRAIT) {
            width = findDesiredDimensionInRange(LANDSCAPE_WIDTH_RATIO, canvasWidth, MIN_FRAME_WIDTH, LANDSCAPE_MAX_FRAME_WIDTH);
            height = findDesiredDimensionInRange(LANDSCAPE_HEIGHT_RATIO, canvasHeight, MIN_FRAME_HEIGHT, LANDSCAPE_MAX_FRAME_HEIGHT);
        } else {
            width = findDesiredDimensionInRange(PORTRAIT_WIDTH_RATIO, canvasWidth, MIN_FRAME_WIDTH, PORTRAIT_MAX_FRAME_WIDTH);
            height = findDesiredDimensionInRange(PORTRAIT_HEIGHT_RATIO, canvasHeight, MIN_FRAME_HEIGHT, PORTRAIT_MAX_FRAME_HEIGHT);
        }

        int leftOffset = (canvasWidth - width) / 2;
        int topOffset = (canvasHeight - height) / 3;
